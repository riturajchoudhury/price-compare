...
SEARCH_SUGGESTIONS = tuple(
    model
    for company_models in PHONE_MODELS_BY_COMPANY.values()
...
5. Close the app — cookies stay in `./browser_session`
    ...
    keyword = st.selectbox(
        ...
        index=None,
        placeholder="Start typing, e.g. Google Pixel 11 Pro",
        accept_new_options=True,
        filter_mode="fuzzy",
        help="Choose a prediction or type any product and press Enter.",
    )
    st.caption(
        f"{len(SEARCH_SUGGESTIONS)} recent models across "
        f"{len(PHONE_MODELS_BY_COMPANY)} major phone companies."
    )
    search_term = (keyword or "").strip()
    run = st.button("Compare prices", type="primary", disabled=not bool(search_term))

    if run and search_term:
